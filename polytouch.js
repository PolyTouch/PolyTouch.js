...
(function(global) {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        var ppin = document.defaultView.getComputedStyle(div, null).getPropertyValue("width");
...
(function(global) {
    var PROPS = [ "altKey", "bubbles", "button", "cancelable", "ctrlKey", "clientX", "clientY", "detail", "fromElement", "isPrimary", "layerX", "layerY", "metaKey", "offsetX", "offsetY", "pageX", "pageY", "pointerId", "pointerType", "pressure", "relatedTarget", "screenX", "screenY", "shiftKey", "target", "tiltX", "tiltY", "timeStamp", "toElement", "view", "x", "y" ];
    function Pointer(id) {
        this.id = id;
        this.events = {};
    }
    Pointer.prototype = {
        ...
        position: function() {
            return this.events.move ? this.events.move[this.events.move.length - 1] : this.events.down[0];
        },
        hasMoved: function(threshold, refEvt) {
            var downEvt = refEvt || this.events.down[0], moveEvt, deltaX, deltaY;
            if (this.events.move) {
...
(function(global) {
    ...
    PointerMap.prototype = {
        ...
        remove: function(id) {
            var idx = this.keys.indexOf(id), cache;
            if (idx > -1) {
...
(function(global) {
    var DEFAULT_PROPERTIES = [ "altKey", "button", "clientX", "clientY", "isPrimary", "layerX", "layerY", "metaKey", "offsetX", "offsetY", "pageX", "pageY", "pointerId", "pointerType", "pressure", "screenX", "screenY", "shiftKey", "target", "tiltX", "tiltY", "view", "x", "y" ];
    function GestureEvent(type, props) {
        var ev = document.createEvent("Event"), p = props || {}, keys, k;
...
(function(global) {
    var TIMEOUT = 300, THRESHOLD = .5;
    var tap = {
        cancel: this.up,
        up: function(pointer, eventData) {
            var downEvt = pointer.events.down[0], props;
            if (downEvt) {
                if (global.math.duration(downEvt.timeStamp, eventData.timeStamp) > TIMEOUT || eventData.target !== downEvt.target || global.gesture.getPointerOnTarget(downEvt.target).length > 1 || pointer.hasMoved(THRESHOLD)) {
                    return;
                }
...
(function(global) {
    ...
    var hold = {
        ...
        detect: function(pointerId) {
            var ptn = global.gesture.pointer.get(pointerId), downEvt, props;
            if (ptn) {
                delete active[pointerId];
                downEvt = ptn.events.down[0];
                if (global.gesture.getPointerOnTarget(downEvt.target).length > 1 || ptn.hasMoved(THRESHOLD)) {
                    return;
                }
                props = global.gesture.cloneProperties(downEvt, global.gesture.DEFAULT_PROPERTIES);
...
(function(global) {
    var PROPS = [ "altKey", "metaKey", "pointerType", "shiftKey", "target", "view" ];
    function scale(s, e) {
        return (Math.sqrt(Math.pow(e[0].x - e[1].x, 2) + Math.pow(e[0].y - e[1].y, 2)) / Math.sqrt(Math.pow(s[0].x - s[1].x, 2) + Math.pow(s[0].y - s[1].y, 2))).toFixed(2);
    }
    function rotation(s, e) {
        return (Math.atan2(e[0].y - e[1].y, e[0].x - e[1].x) * 180 / Math.PI - Math.atan2(s[0].y - s[1].y, s[0].x - s[1].x) * 180 / Math.PI).toFixed(2);
    }
    var THRESHOLD = .25;
    var active = {};
    var pinch = {
        ...
        move: function(pointer, eventData) {
            var cur = active[pointer.id], rel, relPointer, relPosition, props, param;
            if (cur) {
                rel = active[cur.related];
                if (!cur.hasStarted) {
                    if (pointer.hasMoved(THRESHOLD, cur.start)) {
                        cur.hasStarted = true;
                        rel.hasStarted = true;
                ...
                param = cur.primary ? [ [ cur.start, rel.start ], [ eventData, relPosition ] ] : [ [ rel.start, cur.start ], [ relPosition, eventData ] ];
        ...
        cancel: this.up,
        up: function(pointer, eventData) {
            var cur = active[pointer.id], props;
            if (cur) {
                if (cur.hasStarted) {
                    props = global.gesture.cloneProperties(eventData, global.gesture.DEFAULT_PROPERTIES);
